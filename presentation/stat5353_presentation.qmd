---
title: "Transcription to Prediction"
subtitle: "Predicting Survival in AML Patients from RNASeq Data using SVM"
author: "[redacted]"
format: 
    revealjs:
      theme: sakura.scss
...
Among which 571 patients have both RNASeq data (63677 features) and either survived for at least one year after diagnosis (n = 311) or have died (n = 260). We propose a statistical pipeline to predict the one-year survival of AML patients using a support vector machine (SVM) model based on transcriptomic (RNASeq) data of these patients.
...
- We ran the Boruta algorithm for 1000 iterations and reduced 63677 => 60 important features.
:::
...
- Matthews Correlation Coefficient (MCC) [@chicco_matthews_2021] as the primary metric to evaluate the performance of the SVM model.
- $$
...
![Large c vs low c](https://i.stack.imgur.com/GbW5S.png)
...
- The polynomial kernel.
  ...
  expand.grid(
    ...
    mutate(y = (x + c)^d) %>%
    ggplot() +
    geom_line(aes(x = x, y = y, color = as.factor(d)), size = 2) +
    labs(x = "x", y = "K") +
    ggplot_my_theme +
    labs(color = "Degree", x = NULL, y = NULL)
...
### Model and Hyperparameter Tuning (cont'd) [@hsu_practical_nodate]
...
mcc_breaks <- seq(-0.1, 0.5, by=0.05)

svm_boruta_grid_opt_gaussian <- read_csv("../svm_data/svm_grid_opt_boruta_gaussian.csv") %>% 
  filter(!is.na(mcc))

p_gaussian <- svm_boruta_grid_opt_gaussian %>%
  filter(kernel == "gaussian") %>%
  filter(c_pos == c_neg) %>%
  filter(c_pos <= 2^5 & c_neg <= 2^5) %>%
  ggplot(aes(x=c_pos, y=gaussian_eps, z = mcc)) +
  geom_contour_filled(breaks = mcc_breaks) +
  scale_x_continuous(trans = scales::transform_log2(), labels = scales::label_log(base = 2), expand = c(0, 0)) +
  scale_y_log10(limits=c(30,60), expand = c(0, 0)) +
  scale_fill_discrete_sequential(palette = "Viridis", rev = FALSE, drop = FALSE) +
  ggplot_my_theme +
  labs( 
...
svm_boruta_grid_opt_linear <- read_csv("../svm_data/svm_grid_opt_boruta_linear.csv") %>% 
  filter(!is.na(mcc))

p_linear <- svm_boruta_grid_opt_linear %>%
  filter(kernel == "linear") %>%
  filter(c_pos <= 2^5 & c_neg <= 2^5) %>%
  ggplot(aes(x=c_pos,y=c_neg, z = mcc)) +
  geom_contour_filled(breaks = mcc_breaks) +
  scale_x_continuous(trans = scales::transform_log2(), labels = scales::label_log(base = 2), expand = c(0, 0)) +
  scale_y_continuous(trans = scales::transform_log2(), labels = scales::label_log(base = 2), expand = c(0, 0)) +
  scale_fill_discrete_sequential(palette = "Viridis", rev = FALSE, drop = FALSE) +
  ggplot_my_theme +
  labs(
    title = "Linear Kernel Hyperparameters",
    x = "c Value (Positive)",
    y = "c Value (Negative)",
    fill = "MCC")


svm_boruta_grid_opt_polynomial <- read_csv("../svm_data/svm_grid_opt_boruta_polynomial.csv") %>% 
  filter(!is.na(mcc))

p_polynomial <- svm_boruta_grid_opt_polynomial %>%
  filter(kernel == "polynomial") %>%
  filter(c_pos <= 2^5 & c_neg <= 2^5) %>%
  ggplot(aes(x=c_pos,y=polynomial_c, z = mcc)) +
  geom_contour_filled(breaks = mcc_breaks) +
  scale_x_continuous(trans = scales::transform_log2(), labels = scales::label_log(base = 2), expand = c(0, 0)) +
  scale_y_continuous(trans = scales::transform_log2(), labels = scales::label_log(base = 2), expand = c(0, 0)) +
  scale_fill_discrete_sequential(palette = "Viridis", rev = FALSE, drop = FALSE) +
  facet_wrap(~polynomial_degree) +
  ggplot_my_theme +
  labs(
    title = "Polynomial Kernel Hyperparameters",
    caption = "Panel shows degree of polynomial kernel.",
    x = "c Value", 
    y = "Polynomial constant", 
    fill = "MCC")

plot_grid(
  p_linear + theme(legend.position = "none"),
  p_gaussian + theme(legend.position = "none"),
  p_polynomial, 
...
- K-fold is also more robust than the 80/20 split by evaluating the model on different subsets of the data.
:::
:::
...
svm_boruta_grid_opt_gaussian_roc <- read_csv("../svm_data/svm_grid_opt_boruta_gaussian_roc.csv") %>%
  mutate(mode = "Train/Test Split", k = -1)
...
svm_boruta_grid_opt_gaussian_roc %>%
  ...
  ggplot(aes(x=x,y=y, color = mode, group = k)) +
  geom_line(linewidth = 2) +
  coord_fixed() +
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, 1), expand = c(0, 0)) +
  labs(x = "FP%", y="TP%", color = "Mode") +
  ggplot_my_theme +
  theme(legend.position = "bottom")
```
:::
...
::: {.incremental}
- RNASeq data are moderately predictive of one-year survival in AML patients.
- The Boruta algorithm is effective in selecting important features.
- Hyperparameter tuning is crucial for the performance of the SVM model.
:::

## Code Availability, License & References

- The code for reproducing this project is available on [https://github.com/eternal-flame-AD/beataml2.0_svm](https://github.com/eternal-flame-AD/beataml2.0_svm).
